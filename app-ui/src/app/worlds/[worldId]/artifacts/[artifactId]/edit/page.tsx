...
const ITEM_TYPE = "artifacts";

export default function EditArtifactPage({
  params,
}: {
  params: { worldId: string; artifactId: string };
}) {
  const router = useRouter();
  //const { worldId, artifactId } = params;
  ...
  useEffect(() => {
    ...
    getItemById(artifactId).then((data: WorldItem | null) => {
      if (isMounted) {
        setArtifactData(data as ArtifactItem);
        setIsLoading(false);
      }
    });
  ...
  const handleDelete = async () => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to delete ${artifactData?.name}? This action cannot be undone.`
      )
    ) {
      return;
    }
  ...
  const currentartifactName = artifactData.name;
        ...
        <div className="grid gap-8 lg:grid-cols-[320px_minmax(0,1fr)]">
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-4 text-xs text-white/60">
              ...
              <p className="mt-2">
                Pin maps, handouts, or secrets relevant to this artifact.
              </p>
              <button
                type="button"
