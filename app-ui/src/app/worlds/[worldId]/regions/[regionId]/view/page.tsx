...
export default function RegionViewPage() {
  ...
  const imageUrl = region.imageUrl
    ? `${IMAGE_BASE_URL}/${region.imageUrl}`
    : null;
  const galleryImages = region.galleryImages || [];
    ...
    <PageContainer className="space-y-8">
      <header className="space-y-2">
        ...
        <p className="text-sm text-white/65">
          This region dossier is read-only unless the owner shares edit access.
          Ask to be added as a co-author to make changes.
        </p>
      </header>
          ...
          <div className="md:w-1/2">
            {imageUrl ? (
              <div className="w-full rounded-2xl border border-white/10 bg-black/30 overflow-hidden shadow-lg">
                <img
                  src={imageUrl}
                  alt={`Cover image for ${region.name}`}
